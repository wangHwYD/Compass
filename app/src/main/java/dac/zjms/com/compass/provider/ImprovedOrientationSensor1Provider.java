...
import dac.zjms.com.compass.representation.Quaternion;
...
public class ImprovedOrientationSensor1Provider extends OrientationProvider {
     ...
     */
     public float[] temporaryQuaternion = new float[4];
    ...
    public float mDotProd;

    public float[] getTemporaryQuaternion() {
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        if (event.sensor.getType() == Sensor.TYPE_ROTATION_VECTOR) {
            ...
            if (timestamp != 0) {
                ...
                mDotProd = quaternionGyroscope.dotProduct(quaternionRotationVector);

                // If they have diverged, rely on gyroscope only (this happens on some devices when the rotation vector "jumps").
                if (Math.abs(mDotProd) < OUTLIER_THRESHOLD) {
                    // Increase panic counter
                    if (Math.abs(mDotProd) < OUTLIER_PANIC_THRESHOLD) {
                        panicCounter++;
                    }
                ...
                if (panicCounter > PANIC_THRESHOLD) {
                    ...
                    if (gyroscopeRotationVelocity < 3) {
                        Log.d("Rotation Vector",
                                "Performing Panic-reset. Resetting orientation to rotation-vector value."+gyroscopeRotationVelocity);
